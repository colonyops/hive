...
func TestSubPeekDoesNotAcknowledge(t *testing.T) {
	h := NewHarness(t)

	_, err := h.Run("msg", "pub", "--topic", "test.peek", "--sender", "test", "peeked msg")
	require.NoError(t, err)

	// Peek should return the message
	lines, err := h.RunJSONLines("msg", "sub", "--topic", "test.peek", "--peek")
	require.NoError(t, err)
	require.Len(t, lines, 1)
	assert.Equal(t, "peeked msg", lines[0]["payload"])

	// Subsequent sub should still return the same message (not acknowledged)
	lines, err = h.RunJSONLines("msg", "sub", "--topic", "test.peek")
	require.NoError(t, err)
	require.Len(t, lines, 1)
	assert.Equal(t, "peeked msg", lines[0]["payload"])
}

func TestSubLastN(t *testing.T) {
	h := NewHarness(t)

	for i, msg := range []string{"first", "second", "third", "fourth"} {
		_, err := h.Run("msg", "pub", "--topic", "test.lastn", "--sender", "test", msg)
		require.NoError(t, err, "pub message %d", i)
	}

	lines, err := h.RunJSONLines("msg", "sub", "--topic", "test.lastn", "--last", "2")
	require.NoError(t, err)
...
func TestInboxUnreadTracking(t *testing.T) {
	...
	// Read inbox from session dir (acknowledges by default)
...
func TestInboxPeekDoesNotAcknowledge(t *testing.T) {
	h := NewHarness(t)
	repo := createBareRepo(t, "inboxpeek-repo")

	_, err := h.Run("new", "--remote", repo, "inboxpeek-test")
	require.NoError(t, err)
	...
	_, err = h.Run("msg", "pub", "--topic", inboxTopic, "--sender", "test", "peek msg")
	require.NoError(t, err)

	// Peek should not acknowledge
	lines, err := h.RunJSONLinesInDir(dir, "msg", "inbox", "--peek")
	require.NoError(t, err)
	require.Len(t, lines, 1)
	assert.Equal(t, "peek msg", lines[0]["payload"])

	// Regular inbox should still show the message (unread)
	lines, err = h.RunJSONLinesInDir(dir, "msg", "inbox")
	require.NoError(t, err)
	require.Len(t, lines, 1)
	assert.Equal(t, "peek msg", lines[0]["payload"])
...
func TestSubWaitReceivesMessage(t *testing.T) {
	h := NewHarness(t)

	// Publish first, then wait should find it immediately
	_, err := h.Run("msg", "pub", "--topic", "test.wait", "--sender", "test", "waited msg")
	require.NoError(t, err)

	// Use sub with short timeout - message already exists on topic
	// Note: --wait polls for new messages since invocation time, so we use regular sub here
	// and test --wait with a concurrent publish pattern
	lines, err := h.RunJSONLines("msg", "sub", "--topic", "test.wait", "--last", "1")
	require.NoError(t, err)
	require.Len(t, lines, 1)
	assert.Equal(t, "waited msg", lines[0]["payload"])
}
