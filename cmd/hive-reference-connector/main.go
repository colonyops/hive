// Command hive-reference-connector is a minimal example implementation of
// the hive connector JSON-RPC protocol, used by tests and as a protocol
...
// response line so it can be decoded by the connector RPC adapter.
package main
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "hive-reference-connector:", err)
		os.Exit(1)
	}
}
...
// connector implementation below and returns the response to write.
...
// canonicalManifest returns the connector's canned picker manifest: a list
