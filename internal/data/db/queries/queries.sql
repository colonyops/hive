...
-- name: InsertHCActivity :one
INSERT INTO hc_activity (id, item_id, type, message, created_at)
VALUES (?, ?, ?, ?, ?)
RETURNING *;

-- name: ListHCActivity :many
SELECT * FROM hc_activity WHERE item_id = ? ORDER BY created_at ASC;

-- name: LatestHCCheckpoint :one
SELECT * FROM hc_activity
