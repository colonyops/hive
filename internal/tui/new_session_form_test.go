...
func TestNewNewSessionForm(t *testing.T) {
	...
	t.Run("initial name is prefilled and editable", func(t *testing.T) {
		form := NewNewSessionForm(repos, "", "generated-name", nil, nil)
		assert.Equal(t, "generated-name", form.nameInput.Value())
		form.nameInput.SetValue("edited-name")
		assert.Equal(t, "edited-name", form.nameInput.Value())
	})

	t.Run("empty initial name retains placeholder", func(t *testing.T) {
		form := NewNewSessionForm(repos, "", "", nil, nil)
		assert.Empty(t, form.nameInput.Value())
		assert.Equal(t, "my-feature-branch", form.nameInput.Placeholder)
	})
