...
// newIssues constructs the issues connector for tests, failing on spec
// errors.
func newIssues(t *testing.T, exec *fakeExecutor, store kv.KV) *Connector {
	t.Helper()
...
// newPRs constructs the prs connector for tests, failing on spec errors.
func newPRs(t *testing.T, exec *fakeExecutor, store kv.KV) *Connector {
	t.Helper()
...
func TestPRsManifest(t *testing.T) {
	...
	assert.False(t, manifest.Capabilities.FetchDetail, "prs connector has no detail view")
	assert.True(t, manifest.Picker.HidePreview)
	assert.Equal(t, connectors.LayoutModeTable, manifest.Picker.Layout)
...
func TestSearchPRs(t *testing.T) {
	...
	result, err := c.Search(context.Background(), connectors.SearchParams{Scope: "o/r", Query: "feat"})
	require.NoError(t, err)
	require.Len(t, exec.calls, 1)
...
func TestFetchDetailUnsupportedForPRs(t *testing.T) {
	...
	_, err := c.FetchDetail(context.Background(), connectors.FetchDetailParams{ID: "1", Scope: "o/r"})
	require.Error(t, err)
...
func TestCacheIsolatedPerConnector(t *testing.T) {
	store := newTestKV(t)
	...
	issuesResult, err := issues.Search(ctx, connectors.SearchParams{Scope: "o/r"})
	require.NoError(t, err)
	prsResult, err := prs.Search(ctx, connectors.SearchParams{Scope: "o/r"})
	require.NoError(t, err)

	assert.Equal(t, "An issue", issuesResult.Items[0].Title)
	assert.Equal(t, "A PR", prsResult.Items[0].Title, "same scope+query must not collide across connector caches")
}
