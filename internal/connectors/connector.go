// Package connectors defines the domain contract for browsing external
...
// Connector browses an external system and returns items hive can display
// and map into sessions. Implementations may run in-process (e.g. GitHub via
// the gh CLI) or be backed by an external subprocess speaking the connector
// JSON-RPC protocol.
type Connector interface {
	// Name returns the connector's stable identifier (e.g. "github").
	Name() string
	// Available reports whether the connector's runtime dependencies
	// (binaries, auth, etc.) are satisfied.
	Available(ctx context.Context) bool
	// Initialize returns the connector's display/picker manifest.
...
// Manifest describes a connector's identity and how the picker should
// display its items.
...
// Capabilities declares optional connector behavior.
...
// SearchParams carries a search query and scope to a connector.
type SearchParams struct {
	Query string
	Scope string
	// Cursor is an opaque pagination cursor; empty for the first page.
	// Remote connectors may ignore it.
	Cursor string
}
...
// their repository/org. This keeps the contract general for connectors
// added later.
type FetchDetailParams struct {
	ID    string
	Scope string
	// URI is the stable item URI when the connector supplies one; optional.
	URI string
}
