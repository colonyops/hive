...
func TestSubprocessSource_Initialize_HappyPath(t *testing.T) {
	...
	conn, err := NewSubprocessSource("ref", []string{"ref-source"}, runner, time.Second)
	require.NoError(t, err)
...
func TestSubprocessSource_Search_HappyPath(t *testing.T) {
	...
	result, err := conn.Search(context.Background(), sources.SearchParams{Query: "hello", Scope: "scope-a", Cursor: "cursor-1"})
	require.NoError(t, err)
...
func TestSubprocessSource_FetchDetail_HappyPath(t *testing.T) {
	runner := &fakeProcessRunner{
		respond: func(call fakeRunnerCall) ([]byte, []byte, error) {
	...
	detail, err := conn.FetchDetail(context.Background(), sources.FetchDetailParams{ID: "item-1"})
	require.NoError(t, err)
	require.NotNil(t, detail.Markdown)
	assert.Equal(t, "# hi", detail.Markdown.Content)
}

func TestSubprocessSource_IncrementingRequestIDs(t *testing.T) {
	runner := &fakeProcessRunner{
		respond: func(call fakeRunnerCall) ([]byte, []byte, error) {
	...
	require.NoError(t, err)
	_, err = conn.Search(context.Background(), sources.SearchParams{})
	require.NoError(t, err)
...
func TestSubprocessSource_JSONRPCErrorResponse(t *testing.T) {
	runner := &fakeProcessRunner{
		respond: func(call fakeRunnerCall) ([]byte, []byte, error) {
			resp := Response{
				JSONRPC: Version,
...
func TestSubprocessSource_MalformedResponses(t *testing.T) {
	tests := []struct {
		name    string
		stdout  []byte
		wantErr string
...
func TestSubprocessSource_Available(t *testing.T) {
	runner := &fakeProcessRunner{}

	t.Run("resolvable command", func(t *testing.T) {
		conn, err := NewSubprocessSource("ref", []string{"sh"}, runner, time.Second)
		require.NoError(t, err)
		assert.True(t, conn.Available(context.Background()))
	})

	t.Run("unresolvable command", func(t *testing.T) {
		conn, err := NewSubprocessSource("ref", []string{"hive-definitely-not-a-real-binary-xyz"}, runner, time.Second)
		require.NoError(t, err)
		assert.False(t, conn.Available(context.Background()))
	})
}

func TestSubprocessSource_NonZeroExit(t *testing.T) {
	runner := &fakeProcessRunner{
		staticStderr: []byte("source: boom"),
	...
	assert.Contains(t, err.Error(), "source: boom")
}

func TestSubprocessSource_ContextTimeout(t *testing.T) {
	runner := &fakeProcessRunner{honorCtx: true}

	conn, err := NewSubprocessSource("ref", []string{"ref-source"}, runner, 20*time.Millisecond)
	require.NoError(t, err)
...
func TestSubprocessSource_ContextCancellation(t *testing.T) {
	started := make(chan struct{})
	runner := &fakeProcessRunner{honorCtx: true, started: started}

	conn, err := NewSubprocessSource("ref", []string{"ref-source"}, runner, time.Minute)
	require.NoError(t, err)
...
func TestSubprocessSource_LargeStdoutPayload(t *testing.T) {
	...
	detail, err := conn.FetchDetail(context.Background(), sources.FetchDetailParams{ID: "big"})
	require.NoError(t, err)
	require.NotNil(t, detail.Markdown)
	assert.Len(t, detail.Markdown.Content, len(big))
}

func TestNewSubprocessSource_Validation(t *testing.T) {
	runner := &fakeProcessRunner{}

	_, err := NewSubprocessSource("", []string{"cmd"}, runner, time.Second)
	require.Error(t, err)

	_, err = NewSubprocessSource("id", nil, runner, time.Second)
	require.Error(t, err)

	_, err = NewSubprocessSource("id", []string{"cmd"}, nil, time.Second)
	require.Error(t, err)
}

// TestSubprocessSource_ErrorResponseWithNullID verifies that a
// spec-compliant error response with a null id (mandated by JSON-RPC 2.0
// when the server could not parse the request) surfaces the source's
// error message instead of an id-mismatch error.
func TestSubprocessSource_ErrorResponseWithNullID(t *testing.T) {
